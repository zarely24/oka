...
type Props={month:Date;onMonthChange:(m:Date)=>void;months:number;minMonth:Date;maxMonth:Date;from:string;to:string;today:string;dayInfo:(key:string)=>DayInfo;onPick:(key:string)=>void};
...
export default function RangeCalendar({month,onMonthChange,months,minMonth,maxMonth,from,to,today,dayInfo,onPick}:Props){
 ...
 return <div className="ab-cal" onMouseLeave={()=>setHover('')}>
  <button type="button" className="ab-nav ab-prev" disabled={!canPrev} onClick={()=>onMonthChange(addMonths(first,-1))} aria-label="Previous month"><ChevronLeft size={18}/></button>
  <button type="button" className="ab-nav ab-next" disabled={!canNext} onClick={()=>onMonthChange(addMonths(first,1))} aria-label="Next month"><ChevronRight size={18}/></button>
  <div className="ab-months">{Array.from({length:months},(_,i)=>{const m=addMonths(first,i);return <div className="ab-month" key={m.getTime()}>
   <div className="ab-caption">{m.toLocaleDateString('en-GB',{month:'long',year:'numeric'})}</div>
